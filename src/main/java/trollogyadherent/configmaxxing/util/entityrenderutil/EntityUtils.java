package trollogyadherent.configmaxxing.util.entityrenderutil;
...
// @formatter:on
    ...
    public static void drawEntityOnScreenFollowMouse(int posX, int posY, float scale,
    ...
    public static void drawEntityOnScreen(int posX, int posY, float scale, float yaw, float yawOffset, float pitch, EntityLivingBase ent) {
        GL11.glDisable(GL11.GL_BLEND);
        GL11.glDepthMask(true);
        GL11.glEnable(GL11.GL_DEPTH_TEST);
        GL11.glEnable(GL11.GL_ALPHA_TEST);
                                       ...
                                       float targetHeight)
    {
        return (targetHeight / Math.max(Math.max(ent.width, ent.height),
                ent.yOffset /*yOffset2*/)) * baseScale;
    }
